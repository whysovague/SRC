...
import { X, CheckCircle, Globe, Building2, Linkedin, Instagram, Twitter, Phone } from "lucide-react";
export function PartnershipPage({ goToContactForm }: { goToContactForm: () => void }) {  const [showForm, setShowForm] = useState(false);
  const [formStep, setFormStep] = useState<"instructions" | "form">("instructions");
  const [selectedTier, setSelectedTier] = useState("");
  const [submitted, setSubmitted] = useState(false);
  const [formData, setFormData] = useState({
    companyName: "", contactPerson: "", email: "",
    phone: "", website: "", description: "",
    linkedin: "", instagram: "", twitter: "", notes: "",
  });

  const needsDescription = selectedTier === "Strategic" || selectedTier === "Premier";
  ...
  const tierColors: Record<string, string> = {
    Lead: "#4A9BB5", Strategic: ORANGE, Premier: "#C4A882", "Service Partner": TEAL,
  };
    ...
    <div className="relative overflow-hidden pt-24 pb-28">
      <style>{`
        ...
        @keyframes partnerFadeIn {
          from { opacity: 0; transform: perspective(1000px) rotateY(-15deg) scale(0.95); }
          to   { opacity: 1; transform: perspective(1000px) rotateY(0deg) scale(1); }
        }
        .partner-modal { animation: partnerFadeIn 0.4s cubic-bezier(.16,.84,.44,1) both; }
      `}</style>
    ...
    Partners
        ...
        <div className="mb-12">
          ...
          <div className="rounded-2xl p-8 border" style={{ background: "rgba(13,30,48,0.65)", borderColor: "#C4A88230" }}>
            <div className="grid md:grid-cols-2 gap-8 items-center">
              <div>
                <div className="w-44 h-18 rounded-xl flex items-center justify-center mb-5 px-6 py-4"
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5 mt-8 mb-20">
          {[
            ...
            <InteractiveCard key={item.title} accent={item.color}
              className="rounded-2xl p-6 overflow-hidden"
              style={{ background: "rgba(13,30,48,0.6)", border: `1px solid ${item.color}30` }}>
              <div className="w-11 h-11 rounded-xl flex items-center justify-center mb-3" style={{ background: `${item.color}18`, color: item.color }}>
                {item.icon}
              </div>
